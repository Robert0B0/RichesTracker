...
def settings_page(request):
    ...
    if request.method == 'POST':
        form = MonetaryUserForm(
            request.POST, request.FILES, instance=monetaryuser)
        if form.is_valid():
            form.save()
...
def home_page(request):
    records = request.user.monetaryuser.monetaryrecord_set.all()
    goals = request.user.monetaryuser.monetarygoals_set.order_by("due_date")[
        :5]
    total_records = records.count()
    ...
    expenses_records = request.user.monetaryuser.monetaryrecord_set.filter(
        category='expenses')
    unforeseen_records = request.user.monetaryuser.monetaryrecord_set.filter(
        category='unforeseen')
    upkeep_records = request.user.monetaryuser.monetaryrecord_set.filter(
        category='upkeep')
    Investment_records = request.user.monetaryuser.monetaryrecord_set.filter(
        category='Investment')

    monthlyincome_records = request.user.monetaryuser.monetaryrecord_set.filter(
        category='monthly income')
    dividents_records = request.user.monetaryuser.monetaryrecord_set.filter(
        category='dividents')
    others = request.user.monetaryuser.monetaryrecord_set.filter(
        category='other')
